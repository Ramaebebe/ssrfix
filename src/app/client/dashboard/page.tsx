...
export default function DashboardPage() {
  ...
  return (
    <main>
      <div className="grid md:grid-cols-4 gap-5">{kpis.map((k,i)=>(<KpiCard key={i} label={k.label} value={k.value} sub={k.sub} />))}</div>
      <div className="grid md:grid-cols-2 gap-6 mt-6">
        <div className="card p-6 watermark"><h3 className="section-title">Availability (30 days)</h3><AvailabilityChart /></div>
        <div className="card p-6 watermark"><h3 className="section-title">Downtime by Category</h3><DowntimeByCategory /></div>
      </div>
      <div className="card p-6 mt-6 watermark"><h3 className="section-title">Fleet Snapshot</h3><DataTable rows={rows} /></div>
    </main>
  );
}
