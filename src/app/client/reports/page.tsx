...
export default function ReportsPage() {
  ...
  const onImportExcel = async (file: File) => {
    // Dynamically import xlsx to ensure we're on the client and the dep is present
    ...
    const parsedTiles: Tile[] = rows.map((r) => ({
      ...
      spark:
        typeof r.spark === "string"
          ? r.spark.split(",").map((n: string) => Number(n.trim())).filter((n) => !Number.isNaN(n))
          : undefined,
    }));
    ...
    if (sb) {
      try {
        if (parsedTiles.length) {
          await sb.from("report_tiles").upsert(parsedTiles.map((t) => ({ ...t, spark: t.spark ?? [] })));
        }
        if (parsedTx.length) {
          await sb.from("report_transactions").upsert(parsedTx);
        }
      } catch (e) {
